...
a0s_long = np.loadtxt("values_a0_in_Xenon_10_20_10_25_Wcm2_RRon.txt")
num_sims = len(a0s_long)
a0s_short = np.loadtxt("values_a0_in_Xenon_10_20_10_25_Wcm2_RRon.txt", max_rows=1)
os.chdir("a0_" + str(a0s_short))
onefile = np.loadtxt("integration_res_doubleMIDPOINT_new_ne_10minus9_Nr_RRon_powerbase2_maxpower11.txt")
...
for a0value in a0s_long: # a0s_long has len num_sims
    idx += 1 # idx runs from 0 (first iteration, after the increment on this line) to num_sims-1
    results_folder = base_path + "{:.13f}".format(a0value)
    if os.path.exists(results_folder):
        os.chdir(results_folder)
        # os.chdir("a0_" + str(a0value))

        if os.path.isfile("integration_res_doubleMIDPOINT_new_ne_10minus9_Nr_RRon_powerbase2_maxpower11.txt"):
            a = np.loadtxt("integration_res_doubleMIDPOINT_new_ne_10minus9_Nr_RRon_powerbase2_maxpower11.txt") # last column is intensity.
            ...
            if np.any(is_element_nan)==True:
                contor += 1
            intensitiesWcm2[idx] = a[2, -1] # only get the intensity in Wcm2 from the current opened file of results
            ...
            I_4N = remI[-1, :] # the last row is hte most precise one, the most integration samples have been sued for the results appearing on this last row
            I_2N = remI[-2, :]
            I_N = remI [-3, :]
        ...
        a0_values_notexistent.append(a0value)
...
new_num_sims = estimated_res_AITKEN.shape[0]
...
# all_matrices_newTRY = np.zeros( (new_num_sims, 5, num_of_cs_used) )
# all_matrices_newTRY[np.arange(new_num_sims), 0, :] = intensitiesWcm2[np.arange(new_num_sims)].reshape((new_num_sims,1))
# all_matrices_newTRY[np.arange(new_num_sims), 1, :] = estimated_res_AITKEN [np.arange(new_num_sims), :]
# all_matrices_newTRY[np.arange(new_num_sims), 2, :] = estimated_ps_AITKEN[np.arange(new_num_sims), :]
# all_matrices_newTRY[np.arange(new_num_sims), 3, :] = estimated_errs_AITKEN[np.arange(new_num_sims), :]
# all_matrices_newTRY[np.arange(new_num_sims), 4, :] = estimated_errs_RICHARDSON[np.arange(new_num_sims), :]
...
np.save("results_3DimTensor.npy", all_matrices)
